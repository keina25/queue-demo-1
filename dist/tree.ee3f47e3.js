...
var addChild = function addChild(node, value) {
  var newNode = {
    data: value,
    children: null,
    parent: node //往一个节点添加儿子的时候，默认把这个儿子的爸爸parent记为这个节点
...
var travel = function travel(tree, fn) {
  //debugger 
  //有问题可以用这个看一下运行过程
  fn(tree); //fn接受是一个节点，而不是一个节点的data
  //遍历根节点
  ...
  for (var i = 0; i < tree.children.length; i++) {
    travel(tree.children[i], fn); //传进了children，fn这里是传出去
    //这里再遍历子节点的每一个节点
  }
...
var find = function find(tree, node) {
  if (tree === node) {
    return tree; //如果那个节点有再这个根节点里，就回到tree里
    ...
    ;
    return undefined; //如果儿子里面都没找到，就回到undefined
  } else {
      return undefined; //如果不是在根节点和儿子里面，就回到undefined
    }
...
var removeNode = function removeNode(tree, node) {
  var siblings = node.parent.children; //siblings是兄弟姐妹等于爸爸的孩子
  ...
  ;
  siblings.splice(index, 1); //然后从数组里面给删掉
}; //删除节点，不是把这个节点搞没，是把这个节点在这个树的里面的地址搞没


var tree = createTree(10);
var node2 = addChild(tree, 20); //在20这个节点在增加新的节点，
//需先返回newNode，然后node2等于这个值（const）后，
//就可以在20后面加新的节点

var node3 = addChild(tree, 30); //删除node3,先遍历，
